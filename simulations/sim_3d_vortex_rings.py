...
def create_vortex_ring_velocity(
    ...
    # Compute contribution for each staggered face family
    for face_axis in range(3):
        ...
        # Projection onto the ring symmetry axis
...
def run_simulation(
    ...
    reinit_every: int = 2,
    save_every: int = 2,
    output_dir: str = "output_3d_vortex_rings",
):
    """
    Executes the 3D Leapfrog Vortex simulation pipeline.
    """
    nx = resolution
    ny = resolution
    nz = resolution * 2  # Elongated along Z-axis to track travel
    domain_size = 1.0
    ...
    ring_sep = domain_size * 0.15

    # Positioning two coaxial rings
    z1 = nz * dx * 0.35
    center1 = [center_xy[0], center_xy[1], z1]
    z2 = nz * dx * 0.35 + ring_sep
    ...
    max_vel = max(np.max(np.abs(u_x)), np.max(np.abs(u_y)), np.max(np.abs(u_z)), 1e-6)
    dt = 0.3 * dx / max_vel
    ...
    plot_slices(
        ...
        title=f"3D Vortex Rings — Step {num_steps}, |ω|",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="3D Leapfrog Vortex Rings")
